...
export function upperBound<T>(list: T[], value: T, cmp: (x: T, y: T) => Cmp): number {
    let first = 0;
    let len = list.length;
    while (len > 0) {
        let half = len >> 1;
...
export function equalRange<T>(list: T[], value: T, cmp: (x: T, y: T) => Cmp): [number, number] {
    let first = 0;
    let len = list.length;
    while (len > 0) {
        let half = len >> 1;
        ...
        if (cmp(midvalue, value) < 0) {
            first = middle + 1;
            len -= half + 1;
        } else if (cmp(midvalue, value) > 0)
            len = half;
        else {
            let left = first + lowerBound(list.slice(first, middle), value, cmp);
            middle++;
            let right = middle + upperBound(list.slice(middle, first + len), value, cmp);
            return [left, right];
        }
    }
    return [first, first];
