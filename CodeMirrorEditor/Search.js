...
(function(mod) {
	...
	var map = {70: false, 17: false, 16: false, 114: false, 119: false, 72: false} // Key : "F" , "Ctrl", "Shift", "F3", "F8" and "H"
	function doSearch(cm) {
		...
		if (!state.searchDialog){
			var resultObj = complexDialog(cm, queryDialog, "Search for:", q, [function(cm) {findNext(cm, true);}, findNext],
				{
					...
					onKeyDown: function(e, inputs, close) {
						if (e.keyCode in map)
							map[e.keyCode] = true;
						if (map[17] && map[70])
							{doSearch(cm); CodeMirror.e_preventDefault(e);}
						else if (map[16] && map[114])
							{findNext(cm, true); CodeMirror.e_preventDefault(e);}
						else if (map[114])
							{findNext(cm); CodeMirror.e_preventDefault(e);}
						else if (map[119])
							{doReplace(cm); CodeMirror.e_preventDefault(e);}
						else if (map[17] && map[72])
							{replace(cm); CodeMirror.e_preventDefault(e);}
					},
			...
			var inp = document.getElementById("searchDialog")[0];
			if (inp) {
				inp.value = q;
				inp.select();
				inp.focus();
		...
		if (!state.replaceDialog) {
			var resultObj = complexDialog(cm, replaceQueryDialog, "Replace:", "", [doReplace, replaceAll],
				{ 
					...
					onInput: function(e, inputs, close) { state.replace = inputs; },
					onKeyDown: function(e, inputs, close) {
						if (e.keyCode in map)
							map[e.keyCode] = true;
						if (map[17] && map[70]) 
							...
							{ doSearch(cm, true); CodeMirror.e_preventDefault(e); }
						else if (map[114])
							{ doSearch(cm); CodeMirror.e_preventDefault(e); }
						else if (map[119])
							{ doReplace(cm); CodeMirror.e_preventDefault(e); }
					...
					onKeyUp : function(e, inputs, close) { if (e.keyCode in map) map[e.keyCode] = false }
				});
